...
//  fs::path tmpdir = fs::unique_path(fs::temp_directory_path() / "%%%%-%%%%-%%%%-%%%%");
  ...
  BOOST_CHECK_EQUAL(DigestComputer::digestToString(*hash) , "94d988a90c6a3d0f74624368be65e5369ddddb3444841fad4ef41f674b937f26");

  db.UpdateDeviceSeqNo(Name("/lijing"), 1);
  hash = db.RememberStateInStateLog();

  BOOST_CHECK_EQUAL(DigestComputer::digestToString(*hash), "91a849eede75acd56ae1bcb99e92d8fb28757683bc387dbb0e59c3108fcf4f18");

  db.UpdateDeviceSeqNo(Name("/lijing"), 2);
  hash = db.RememberStateInStateLog();
  BOOST_CHECK_EQUAL(DigestComputer::digestToString(*hash), "d2dfeda56ed98c0e17d455a859bc8c3b9e31c85c138c280a8badab4fc551f282");
  ...
  msg = db.FindStateDifferences("00", "d2dfeda56ed98c0e17d455a859bc8c3b9e31c85c138c280a8badab4fc551f282");
  BOOST_CHECK_EQUAL(msg->state_size(), 1);
  BOOST_CHECK_EQUAL(msg->state(0).type(), SyncState::UPDATE);
  BOOST_CHECK_EQUAL(msg->state(0).seq(), 2);

  msg = db.FindStateDifferences("d2dfeda56ed98c0e17d455a859bc8c3b9e31c85c138c280a8badab4fc551f282", "00");
  BOOST_CHECK_EQUAL(msg->state_size(), 1);
  ...
  hash = db.RememberStateInStateLog();
  BOOST_CHECK_EQUAL(DigestComputer::digestToString(*hash), "602ff1878fc394b90e4a0e90c7409ea4b8ee8aa40169801d62f838470551db7c");

  msg = db.FindStateDifferences("00", "602ff1878fc394b90e4a0e90c7409ea4b8ee8aa40169801d62f838470551db7c");
