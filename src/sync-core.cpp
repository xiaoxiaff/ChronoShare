  ...
  , m_syncInterestInterval(syncInterestInterval)
{
  ...
  time::seconds interval =
    time::seconds((m_syncInterestInterval > 0 && m_syncInterestInterval < 30) ?
                  m_syncInterestInterval : 4);

  m_periodicInterestEvent = m_scheduler.scheduleEvent(interval,
    bind(&SyncCore::sendPeriodicSyncInterest, this, interval));

  m_syncInterestEvent = m_scheduler.scheduleEvent(time::milliseconds(100),
...
SyncCore::localStateChanged()
{
  ...
  m_syncInterestEvent = m_scheduler.scheduleEvent(time::milliseconds(50),
                                                  bind(&SyncCore::sendSyncInterest, this));
...
SyncCore::localStateChangedDelayed()
{
  ...
  m_localStateDelayedEvent = m_scheduler.scheduleEvent(time::milliseconds(500),
...
SyncCore::handleSyncData(const Interest& interest, Data& data)
{
  ...
  m_syncInterestEvent = m_scheduler.scheduleEvent(time::milliseconds(0),
                                                  bind(&SyncCore::sendSyncInterest, this));
}
