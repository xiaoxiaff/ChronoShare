...
StateServer::registerPrefixes()
{
  ...
  actionsFolderId =
    m_face->setInterestFilter(ndn::InterestFilter(actionsFolder),
  ...
  actionsFileId =
    m_face->setInterestFilter(ndn::InterestFilter(actionsFile),
  ...
  filesFolderId =
    m_face->setInterestFilter(ndn::InterestFilter(filesFolder),
  ...
  restoreFileId =
    m_face->setInterestFilter(ndn::InterestFilter(restoreFile),
...
void
StateServer::formatActionJson(json_spirit::Array& actions, const ndn::Name& name,
...
StateServer::formatFilestateJson(json_spirit::Array& files, const FileItem& file)
{
  ...
  json.push_back(Pair("hash", DigestComputer::digestToString(
                                ndn::Buffer(file.file_hash().c_str(), file.file_hash().size()))));
...
StateServer::cmd_restore_file_Execute(const ndn::Name& interest)
{
  ...
  //    	ndn::Name deviceName = ndn::Name(file->device_name().c_str()).getSubName(0,
  //    file->device_name().size());
  ndn::Name
  deviceName(
    ndn::Block((const unsigned char*)file->device_name().c_str(), file->device_name().size()));
