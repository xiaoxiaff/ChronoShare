...
$.Class ("ChronoShare", { },
 {
     init: function (username, foldername) {
         ...
         this.files = new Name ("/localhop").add (this.username).add ("chronoshare").add (foldername).add ("info").add ("files").add ("folder");

         this.actions = new Name ("/localhop").add (this.username).add ("chronoshare").add (foldername).add ("info").add ("actions");

         this.restore = new Name ("/localhop").add (this.username).add ("chronoshare").add (foldername).add ("cmd").add ("restore").add ("file");
     ...
     get_file: function (modifiedBy, hash, segments, callback/*function (bool <- data received, data <- returned data)*/) {
         ...
         request = new Name ()
            .add (baseName)
            .addSegment (lastSegmentRequested);
...
function file_onData(interest, upcallInfo) {
  ...
  if (lastSegmentRequested+1 == file_segments) {
    ...
    nextSegment = interest.getName().getPrefix(-1).addSegment(lastSegmentRequested);
...
function info_actions_onData(interest, upcallInfo) {
  ...
  convertedData = upcallInfo.getContent().buf().toString('binary'); //DataUtils.toString (upcallInfo.contentObject.content);
  console.log(convertedData);
  moreName = "more";
